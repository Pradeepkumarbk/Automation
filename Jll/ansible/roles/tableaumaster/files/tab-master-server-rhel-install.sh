...
option_init() {
  ...
  while getopts "u:p:i:t:k:i:a:s:-:" opt; do
    case "$opt" in
      u)
        clientid=$OPTARG
        ;;
...
final_setup() {
  ...
  tabcmd initialuser --server 'localhost:80' --username 'provisioner' --password "${tab_admin_password}"
  echo "=================="
...
azcli()
{
    ...
    export AZURE_STORAGE_KEY=$accesskey
}
...
az storage blob upload -c tableau  --account-name $storageaccount  -f bootstrap.json -n bootstrap.json
finish
